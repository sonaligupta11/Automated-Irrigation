...
const long criticalThreshold=60;   // point where moisutre content is critical is 50%
...
const long delayTime = 30000;      // Pause in the loop  (millisecs)
...
const long minMoisture=380;     // Full saturation, sensor reading is at the lowest reading is 200
...
long moisture, moistureRead=0;                // Store light from SoilSensor (250-1023)
...
void loop(){  
  ...
  moistureRead = analogRead(soilSensor);
  moisture = map(constrain(moistureRead,minMoisture,maxMoisture),maxMoisture,minMoisture,0,100);
...
void writeState(boolean isDry){                   // Write output to OLED
  ...
  //writeMsg(1,state+"-"+String(moistureRead)+" - " + String(moisture)+"%-"+String(light));
